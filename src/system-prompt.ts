...
export function buildSystemPromptBlock(args: SystemPromptArgs): string {
	...
	const lines = [
		...
		"    Treat `explore` as the reconnaissance owner: use blocking `subagent_run` or foreground `Agent`, then wait before reading/searching the same code.",
		...
		"  - Claude-style: `Agent` launches a sub-agent; use `run_in_background: true` for background work. `explore` ignores it and blocks.",
		"  - Default: `subagent_dispatch` (background) — returns immediately, you keep working. `explore` is coerced to blocking.",
		...
		"",
		"Tracking:",
		"  - Prefer background completion notifications and blocking `subagent_run`/foreground `Agent` results; do not fetch the same result again.",
